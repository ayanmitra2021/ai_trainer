...
- [x] 18.1 DB schema — `byte_sized_lessons` and `lesson_reads` tables + migration
- [x] 18.2 `ByteSizedLessonAgent` — generates crisp, bulleted, engagement-rich write-up per skill gap with curated external links 👤
- [x] 18.3 API routes — generate lessons (background), list with history, open/close read sessions
- [x] 18.4 Byte-Sized Learning table UI — above Learning Journey in Skill Radar tab; unread rows glow; historical path lessons preserved below divider
- [x] 18.5 Read modal — full write-up in popup, circular clock timer, time-spent recording on close, "read again" nudge if closed too early
- [x] 18.6 Read Aloud + playback speed — Web Speech API TTS with 0.75×/1×/1.25×/1.5×/2× speed control
- [x] 18.7 Always-available Mock Exam — remove 80% mastery gate; soft advisory tip shown when mastery < 40%

**Phase 19 — Mock Exam Intelligence: History, Abandonment & Confidence Score**
- [x] 19.1 DB schema — `abandoned_reason`, `abandoned_at` on `mock_exam_sessions`; `'abandoned'` status in check constraint; Alembic migration
- [x] 19.2 Abandon API + exam history list — `POST /mock-exams/{id}/abandon` (requires non-empty reason); `GET /mock-exams` returns all sessions newest-first; active-session guard updated to include `'abandoned'`-is-inactive logic
- [x] 19.3 Smart question recycling — new exam generation checks for recyclable questions per domain before calling the LLM: (a) unanswered questions from abandoned sessions get top priority; (b) incorrectly-answered questions from any prior session are included randomly (up to 30% of domain slot); (c) LLM called only for remaining slots; recycled questions are re-shuffled into the new session
...
**Why this phase exists.**  Quiz scores and domain gap charts tell a practitioner *where* their gaps are — but not what to do about them right now. Phase 18 closes that loop with on-demand, AI-generated micro-content: a short, engaging write-up per skill gap that a practitioner can read (or listen to) in one focused sitting. The content is regenerated each time the practitioner regenerates their path, so it stays calibrated to current gaps. Historical lessons are preserved so nothing is lost across path generations. Separately, the 80%-mastery gate on Mock Exam is removed — practitioners should be able to stress-test themselves at any point, not just once the system deems them "ready."

**Preconditions:** Phase 17 Definition of Done is met.

---

### Step 18.1 — DB schema: `byte_sized_lessons` and `lesson_reads` tables + migration

**Goal:** introduce the two tables that back the Byte-Sized Learning feature.

**Context to load:** `docs/data-model.md`, `backend/alembic/versions/` (latest migration number).
...
- `learning_path_id` (FK → learning_paths — the specific path generation this lesson belongs to)
- `skill_id` (FK → skills)
- `skill_name` (text — denormalized for display; skill names rarely change)
- `gap_pct` (float — current mastery score expressed as a gap, i.e. `1 - mastery_score`, at generation time; range 0–1)
- `target_pct` (float — practitioner's implied target, default 0.85 / 85 %; can be overridden by cert passing score threshold)
- `what_missing` (text — 1–2 sentence plain-English summary of the gap, shown in the table without opening the modal)
- `content_md` (text — full markdown write-up, max ~750 words / ~5 min read; generated by ByteSizedLessonAgent)
- `external_links` (JSONB — ordered list of `{title: text, url: text, type: 'blog'|'docs'|'video'}`, 3–5 curated links at end of write-up)
- `estimated_read_minutes` (smallint — LLM-estimated read time, 1–5 minutes)
- `path_generation_seq` (integer — monotonically-increasing counter per practitioner; incremented each time a new path is generated; all lessons from the same generation share the same seq value; used to group current vs. historical lessons)
- `generation_status` (text NOT NULL, default `'pending'` — `'pending' | 'ready' | 'failed'`; mirrors the quiz generation lifecycle)
- `created_at`
...
- `duration_seconds` (integer nullable — NULL until the modal is closed; set to elapsed seconds on close)
- `created_at`

*ORM models, Pydantic schemas, and `alembic upgrade head` immediately after writing the migration.*

**Scenario tests:**
- *Migration is idempotent* — `alembic upgrade head` twice; second run makes no changes.
- *`lesson_reads.duration_seconds` nullable* — a lesson read row with `duration_seconds = NULL` can be inserted without error.
- *`path_generation_seq` isolates history* — given two lessons with `path_generation_seq = 1` and one with `seq = 2`, a query filtering on the max seq returns only the one row.

**Definition of done:** migration runs clean locally; all three scenario tests pass.

---

### Step 18.2 — `ByteSizedLessonAgent` — engaging write-up generator 👤

**Goal:** an LLM agent that, given a skill's name, current mastery gap, cert context, and the skill's exam-domain description, produces a short, fun, and genuinely useful learning write-up with curated external links.

**Context to load:** `docs/architecture.md` (Agent contract), `docs/coding-guidelines.md`, `backend/app/agents/base.py`, `docs/human-in-the-loop.md`.

**Build:**

1. **`backend/app/agents/byte_sized_lesson.py`** — follows the standard `Agent[TInput, TOutput]` contract.

   *Input (`ByteSizedLessonInput`):*
   - `skill_name: str`
   - `skill_description: str` (from `skills.description`)
   - `current_mastery_score: float` (0–1)
   - `target_mastery_score: float` (default 0.85)
   - `certification_name: str`
   - `domain_name: str` (the cert exam domain this skill primarily tests, e.g. "Fundamentals of Generative AI — 24%")
   - `domain_description: str`

   *Output (`ByteSizedLessonOutput`):*
   - `what_missing: str` — 1–2 sentence plain-English summary for the table column (not Markdown)
   - `content_md: str` — full write-up in Markdown; must include: short hook paragraph (1–2 sentences), a "What you need to know" bulleted list (the core concepts), a "Common pitfalls 🪤" section (2–3 misconceptions to avoid), a "Quick check ✅" section (2–3 self-test questions the reader can answer mentally), and a sign-off line like "You're ready to tackle this on exam day!"
   - `external_links: list[ExternalLink]` — 3–5 items, each `{title, url, type}` where `type` is `'blog'`, `'docs'`, or `'video'`; must include at least one official vendor/provider source (e.g. Anthropic docs, AWS docs, Microsoft Learn)
   - `estimated_read_minutes: int` — LLM-estimated read time in whole minutes (1–5)

2. **`backend/app/agents/prompts/byte_sized_lesson.md`** 👤 — written by Ayan (see `docs/human-in-the-loop.md`). The prompt must instruct the LLM to: write conversationally, use emojis sparingly but effectively, keep sentences short, avoid jargon without explanation, make the "Common pitfalls" section genuinely insightful (not generic), and only include external links to real and reputable sources — Anthropic docs, AWS docs, Microsoft Learn, Google Cloud documentation, or highly-cited technical blog posts.

3. **Model:** Haiku via the standard `MultiTierModelClient` (same cost-controlled path as quiz generation). Content quality is high enough for Haiku; the structured output schema keeps it disciplined.

4. **`max_tokens`: 2000** — covers the full write-up plus links comfortably within NVIDIA's per-call budget.

**Scenario tests (stub client):**
- *Agent produces a complete, schema-valid output* — Given a stub that returns a valid `ByteSizedLessonOutput`, when the agent runs, then all required fields are present and `estimated_read_minutes` is between 1 and 5.
- *`external_links` list is non-empty* — Given the stub output, then `len(external_links) >= 3`.
- *An `agent_runs` row is persisted* — Given a successful run, then one `agent_runs` row exists with `agent_name = 'byte_sized_lesson'` and `status = 'success'`.

**Definition of done:** all three scenario tests pass against the stub client.

> 👤 **Human-in-the-loop:** write `agents/prompts/byte_sized_lesson.md` yourself — specifically the engagement style instructions (conversational tone, emoji policy, hook structure), the "Common pitfalls" guidance (what makes a pitfall genuinely insightful vs. generic), and the link-quality rules (what sources count as reputable, what to do if a real source cannot be found). The write-up's quality and personality is the practitioner's primary touchpoint with this feature; a bland prompt produces bland content, which defeats the purpose. See `docs/human-in-the-loop.md`.

---

### Step 18.3 — API routes: generate, list, and read-session endpoints

**Goal:** expose the lesson lifecycle over the REST API — generation (background), retrieval (with history), and read-session tracking (open + close).

**Context to load:** `backend/app/api/routes/` (existing route files), `docs/data-model.md`.

**Build:**

**`POST /practitioners/{id}/byte-sized-lessons/generate`**
- Triggered automatically as a FastAPI `BackgroundTask` at the end of `POST /learning-paths/generate` (hook into the same background launcher that handles quiz generation).
- Can also be called standalone (e.g. admin recovery).
- Determines the new `path_generation_seq` by incrementing the current max for this practitioner (SELECT MAX + 1, default 1 if none).
- For each skill in the current active learning path (joined to their mastery scores and cert domain), creates one `byte_sized_lessons` row with `generation_status='pending'`, then calls `ByteSizedLessonAgent` to fill it in; on success sets `generation_status='ready'`; on `AllProvidersUnavailableError` sets `generation_status='failed'`.
- Per-skill failures are isolated (log WARNING, continue) — same pattern as quiz generation.
- Uses a fresh DB session (background task runs after the request session is closed).
- Returns `{"started": true, "path_generation_seq": N}` synchronously; the background task updates rows asynchronously.

**`GET /practitioners/{id}/byte-sized-lessons`**
- Returns all lessons for this practitioner, grouped:
  - `current`: list of lessons where `path_generation_seq = MAX(path_generation_seq)` — ordered by `gap_pct DESC` (largest gap first).
  - `history`: list of lessons where `path_generation_seq < MAX`, ordered by `path_generation_seq DESC`, then `gap_pct DESC`.
- Each lesson object includes: `id`, `skill_id`, `skill_name`, `gap_pct`, `target_pct`, `what_missing`, `estimated_read_minutes`, `generation_status`, `path_generation_seq`, `total_read_seconds` (sum of `lesson_reads.duration_seconds` for this lesson, NULL if never read), `last_read_at` (MAX `lesson_reads.started_at`, NULL if never read).
- `content_md` and `external_links` are **not** returned in the list — fetched only when the modal opens.

**`GET /practitioners/{id}/byte-sized-lessons/{lesson_id}`**
- Full lesson detail including `content_md` and `external_links`.
- Also returns the same `total_read_seconds` and `last_read_at` computed fields.

**`POST /practitioners/{id}/byte-sized-lessons/{lesson_id}/read-sessions`**
- Opens a new read session. Creates a `lesson_reads` row with `started_at = now()`, `duration_seconds = NULL`.
- Returns `{"session_id": UUID, "started_at": timestamp}`.

**`PATCH /practitioners/{id}/byte-sized-lessons/{lesson_id}/read-sessions/{session_id}`**
- Closes a read session. Accepts `{"duration_seconds": int}`. Sets `lesson_reads.duration_seconds`.
- Idempotent if `duration_seconds` is already set (second PATCH is a no-op returning 200).

**Scenario tests:**
- *Lesson generation runs as a background task* — Given a path with 3 skills, when `generate` is called, then 3 `byte_sized_lessons` rows are created and the HTTP response returns before all agent calls finish.
- *List endpoint separates current and historical groups* — Given a practitioner with lessons at `path_generation_seq = 1` and `seq = 2`, the response has `current` (seq=2 only) and `history` (seq=1 only).
- *`content_md` absent from list, present in detail* — `GET /byte-sized-lessons` does not include `content_md`; `GET /byte-sized-lessons/{id}` does.
- *Read session lifecycle* — POST opens a session (duration=NULL); PATCH closes it (duration set); second PATCH is a no-op.
- *`total_read_seconds` aggregates multiple read sessions* — given two closed read sessions for the same lesson (60 s + 90 s), the list endpoint returns `total_read_seconds = 150`.

**Definition of done:** all five scenario tests pass.

---

### Step 18.4 — Byte-Sized Learning table UI (Skill Radar tab)

**Goal:** surface the lessons as an actionable table in the Skill Radar tab, above the existing Learning Journey section. Unread lessons glow to draw attention; historical lessons sit below a clear divider.

**Context to load:** `frontend/src/components/SkillRadar/SkillRadar.tsx` (or the relevant tab component), `frontend/src/api/types.ts`.

**Build:**

1. **`ByteSizedLearningTable` component** — new file at `frontend/src/components/SkillRadar/ByteSizedLearningTable.tsx`.

   *Table columns:*
   | Column | Source | Notes |
   |---|---|---|
   | Skill | `skill_name` | Plain text |
   | Current Gap % | `gap_pct * 100` | Formatted as "42%" |
   | Target % | `(1 - target_pct) * 100` → shown as gap remaining | Or display target mastery: `target_pct * 100` + "% mastery" |
   | What You Might Be Missing | `what_missing` | 1–2 sentences; truncated to ~80 chars with tooltip on hover if longer |
   | Time Spent | Computed from `total_read_seconds` | See display rules below |
   | Read | Button | Opens modal (Step 18.5) |

   *Time Spent display rules:*
   - `total_read_seconds = NULL` → "—" (never opened)
   - `total_read_seconds > 0` but < `estimated_read_minutes * 60 * 0.5` → "⚡ Read again" (amber, pulsing badge)
   - `total_read_seconds >= estimated_read_minutes * 60 * 0.5` → e.g. "2 min 14 sec" (green text)

   *Unread row highlight:* rows where `total_read_seconds = NULL` (never read) and `generation_status = 'ready'` get a pulsing/glowing left border in the accent color (CSS `@keyframes pulse-border` — 1.5 s ease-in-out, opacity oscillating between 0.4 and 1.0 on the border-left color). This uses only CSS; no JavaScript animation loop.

   *`generation_status = 'pending'` rows:* show "⏳ Generating…" in the Read column instead of a button.
   *`generation_status = 'failed'` rows:* show "⚠️ Failed" in the Read column with a small Retry link that calls `POST .../generate` for just that skill.

2. **Section header:** "📖 Byte-Sized Learning" with a subtitle "Targeted micro-reads for your current gaps — regenerated each time you update your path." Placed above the existing Learning Journey section in the Skill Radar tab.

3. **Current vs. history layout:**
   - Current lessons (latest `path_generation_seq`) displayed first in the table.
   - A section divider "Previous paths" separates historical lessons below. Historical rows are styled with reduced opacity (0.65) and no glow animation — they are reference, not action items.
   - If no historical lessons exist, the divider is omitted.

4. **Polling:** poll `GET /byte-sized-lessons` every 5 s while any current lesson has `generation_status = 'pending'`; stop once all are `ready` or `failed` (mirrors quiz polling pattern).

**Scenario tests (Playwright):**
- *A practitioner with a path and generated lessons sees the table above the Learning Journey section.*
- *An unread lesson row has a visible glowing/pulsing left border; a read lesson row does not.*
- *A lesson with `total_read_seconds < 50% of estimated_read_minutes * 60` displays "⚡ Read again" in the Time Spent column.*

**Definition of done:** all three Playwright scenarios pass.

---

### Step 18.5 — Read modal: write-up, circular timer, time-spent recording

**Goal:** clicking "Read" opens a modal that shows the lesson content, tracks how long the practitioner spends, and records the time when closed. If closed too early, the table entry is updated to nudge a re-read.

**Context to load:** `frontend/src/components/SkillRadar/ByteSizedLearningTable.tsx` (Step 18.4).

**Build:**

1. **`LessonModal` component** — `frontend/src/components/SkillRadar/LessonModal.tsx`.

   *Modal layout (top → bottom):*
   - **Header row:** skill name as title (left), circular clock timer (right) + "🔊 Read Aloud" button + close (×) button.
   - **Body:** `content_md` rendered as Markdown (use the existing markdown renderer already in the project, or `react-markdown` if already a dependency).
   - **Footer:** "📚 Want to learn more?" section with the `external_links` rendered as a list of labeled links with type icons: 📝 blog, 📖 docs, 🎥 video. Links open in a new tab (`target="_blank" rel="noopener noreferrer"`).

2. **Circular clock timer:**
   - SVG circle (stroke-dasharray / stroke-dashoffset technique) that fills in as time passes. The circle completes one full revolution per `estimated_read_minutes` minutes — if the lesson is estimated at 3 minutes, the circle is fully filled at 3:00.
   - Centre of the circle shows elapsed time in `M:SS` format (e.g. "1:42").
   - After the estimated time is reached, the circle stays full and the timer colour changes from blue to green — a "you've read enough" signal. Timer continues counting.
   - Implemented with `setInterval(1000)` inside a `useEffect`; the interval is cleared on modal unmount.

3. **Open/close lifecycle:**
   - On modal open: call `POST .../read-sessions` to get a `session_id`; store in component state.
   - On modal close (× button or Escape key): call `PATCH .../read-sessions/{session_id}` with the elapsed `duration_seconds`; then invalidate the `GET /byte-sized-lessons` query so the table refreshes immediately.

4. **"Read again" nudge:** the `total_read_seconds` threshold check (`< 50% of estimated_read_minutes * 60`) is computed on the backend list response and displayed in the table — the modal itself does not need to make this judgment. The table re-renders as soon as the query is invalidated after close.

**Scenario tests (Playwright):**
- *Opening the modal fires a POST read-session; closing it fires a PATCH with duration_seconds > 0.*
- *The circular timer reaches the estimated-read circle-fill at the correct elapsed time (fast-forward by mocking `Date.now`).*
- *After closing with short duration, the "⚡ Read again" indicator appears in the table row.*

**Definition of done:** all three Playwright scenarios pass; manual test confirms the timer ticks and the table updates on close.

---

### Step 18.6 — Read Aloud with playback speed control

**Goal:** a "🔊 Read Aloud" button in the modal header reads the lesson content aloud using the browser's Web Speech API (`speechSynthesis`). The practitioner can control playback speed exactly as on YouTube (0.75× / 1× / 1.25× / 1.5× / 2×).

**Context to load:** `frontend/src/components/SkillRadar/LessonModal.tsx` (Step 18.5). No backend changes — this is entirely browser-side.

**Design rationale:** Web Speech API (`window.speechSynthesis`) is available in all modern browsers (Chrome, Edge, Firefox, Safari) with no external service call and no API key. The text read is the lesson's `content_md` stripped of Markdown syntax (headings, bullets, bold/italic markers, link syntax) before being passed to `SpeechSynthesisUtterance`. External links are not read aloud.

**Build:**

1. **`useSpeechSynthesis` hook** — `frontend/src/hooks/useSpeechSynthesis.ts`:
   - `speak(text: string, rate: number)` — cancels any in-progress utterance, creates a new `SpeechSynthesisUtterance`, sets `.rate`, calls `window.speechSynthesis.speak()`.
   - `stop()` — calls `window.speechSynthesis.cancel()`.
   - `isPlaying: boolean` — derived from `onstart` / `onend` / `onerror` event listeners on the utterance.
   - Cleanup on unmount: `window.speechSynthesis.cancel()`.

2. **Markdown-to-plain-text utility** — `frontend/src/utils/markdownToPlain.ts`: strips `#`, `*`, `_`, `[text](url)` → `text`, `` ` ``, `>` prefixes, and bare URLs. Simple regex, no parser dependency needed for this use.

3. **Speed selector in `LessonModal`** — a compact `<select>` or five toggle buttons next to the 🔊 button: **0.75×**, **1×** (default, pre-selected), **1.25×**, **1.5×**, **2×**. Changing speed while playing: `stop()` then `speak()` with new rate (the browser does not support live rate change on an in-progress utterance).

4. **Button state:** when TTS is playing, the button label changes to "⏹ Stop"; clicking it calls `stop()`. When stopped, it returns to "🔊 Read Aloud".

5. **Graceful fallback:** if `window.speechSynthesis` is undefined (some server-side-rendered environments or blocked by browser settings), hide the button entirely — do not show an error.

**Scenario tests:** manual verification only (Web Speech API is not available in Playwright's headless Chromium by default without extra configuration).
- Confirm: clicking "🔊 Read Aloud" begins speech in a real browser; speed selector changes the perceived rate; clicking ⏹ Stop halts it; closing the modal stops any in-progress speech.

**Definition of done:** all manual checks pass in Chrome and Edge. The button is hidden when `speechSynthesis` is unavailable.

---

### Step 18.7 — Always-available Mock Exam (remove 80% mastery gate)

**Goal:** the Mock Exam CTA / button in the Skill Radar tab is always visible to the practitioner, regardless of their current aggregate mastery score. The 80%-mastery gate introduced in Step 11.4 is removed. All other mock exam mechanics — question count per cert, duration, passing score threshold, timer, instant per-question feedback, completion recording — are unchanged.

**Context to load:** `frontend/src/components/SkillRadar/SkillRadar.tsx` (or the relevant component where the 80% CTA lives), `frontend/src/pages/MockExamPage.tsx`.

**Build:**

1. **Remove the mastery gate in the frontend** — find the conditional that hides the Mock Exam button when `aggregateMastery < 0.80` and remove it. The button is rendered unconditionally.

2. **Add a soft advisory tip** — when `aggregateMastery < 0.40`, show a small hint below the Mock Exam button:
   ```
   💡 Tip: answering more quizzes first will sharpen your readiness before the mock exam — but you're welcome to try anytime!
   ```
   Between 40% and 80%, no tip is shown (practitioners at this level are expected to attempt the mock exam). At 80% and above, show the existing "you look ready" message (Step 6.7 guidance message).

3. **No backend changes required** — the mock exam API, `MockExamGenerator` agent, and `exam_sessions` table are unchanged. The backend never enforced the 80% gate; it was purely a frontend display condition.

4. **Update the guidance message thresholds** in `SkillRadar` to reflect the new design:
   - **< 40%:** existing "Keep building" message + the new advisory tip below the Mock Exam button.
   - **40–69%:** existing "Good progress" message; Mock Exam button visible without any tip.
   - **70–89%:** existing "You're getting close" message; Mock Exam button visible.
   - **≥ 90%:** existing "Strong profile" message; Mock Exam button visible with "you look ready" styling.

**Scenario tests (Playwright):**
- *A practitioner with 20% mastery sees the Mock Exam button and the advisory tip.*
- *A practitioner with 55% mastery sees the Mock Exam button without any tip.*
- *Clicking the Mock Exam button at 20% mastery navigates to MockExamPage without error.*

**Definition of done:** all three Playwright scenarios pass; manual test confirms the exam page loads and the timer starts correctly from a sub-80% state.
